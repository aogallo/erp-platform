...
Add HR organizational master data as a new HR backend slice that follows the current scaffold pattern: pure dataclass domain objects, service/repository `Protocol` contracts, explicit schema shapes, FastAPI route scaffolds, and Unit of Work ports. HR owns organizational units, positions, reporting lines, assignments, and allocation records. Accounting owns cost centers and exposes only a validation/read boundary for HR allocation references. No approval workflow runtime is implemented; the HR contract only exposes vacancy and superior-position data needed by future approval consumers.

## Architecture Decisions

| Decision | Choice | Alternatives considered | Rationale |
|---|---|---|---|
| HR owns positions | Model `OrganizationalUnit`, `Position`, `PositionReportingLine`, `PositionAssignment`, and `PositionCostAllocation` under `backend/src/hr/domain/`. | Store positions in IAM roles or Accounting dimensions. | Specs require role-independent HR ownership; IAM stays identity/RBAC-only and Accounting stays finance catalog owner. |
...
For a position allocation, the service verifies HR permission and tenant scope, loads the position, validates each cost center through Accounting, enforces active references and 100% Decimal percentage total, then stores cost-center database IDs while returning readable codes in read models.
...
| `docs/bounded-context.md` | Modify | Add units, positions, cost allocation references, and Accounting cost-center boundary to HR/Accounting context map. |
...
Domain invariants: every record carries `tenant_id`; allocation percentages use `Decimal` and total exactly `Decimal("100")`; one person/employee can have only one active position assignment at a time; vacancy is valid and exposed without creating approval requests.
...
Phase 1 adds import-safe contracts, route scaffolds, docs, and tests. Phase 2 adds reviewed SQL migrations for `hr_organizational_units`, `hr_positions`, `hr_position_reporting_lines`, `hr_position_assignments`, `hr_position_cost_allocations`, and `accounting_cost_centers`, with foreign keys from HR allocation rows to Accounting cost centers and a uniqueness/exclusion strategy for one active assignment per person. Rollout is additive; no existing data migration is required.
