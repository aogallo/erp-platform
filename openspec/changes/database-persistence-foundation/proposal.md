...
Define the shared PostgreSQL, Flyway, SQLAlchemy, and Unit of Work foundation before Accounting persistence work. The repo has persistence contracts but no ORM models, SQL migrations, Flyway config, or concrete async transaction boundary.
...
- SQLAlchemy async engine/session setup, metadata strategy, and concrete Unit of Work boundary.
...
### New Capabilities
- `database-persistence-foundation`: Migration structure, schema baseline, UUID/audit/tenant conventions, SQLAlchemy async setup, and Unit of Work behavior.
...
Use PostgreSQL schemas per bounded context plus `tenant_id` on tenant-scoped tables. Keep global/system catalogs tenantless only when explicitly global. Flyway owns reviewable SQL migrations and enables `pgcrypto` before UUID defaults. Shared SQLAlchemy async session infrastructure and UoW own commit/rollback; controllers and repositories must not commit directly.
...
| `backend/src/shared/uow/` | Modified | Concrete async transaction boundary. |
...
- Existing IAM tenant/principal contracts and Unit of Work policy.
...
- [ ] Specs define async SQLAlchemy session setup and UoW ownership.
